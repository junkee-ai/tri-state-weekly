...
export default function AuthModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
      ...
      if (signUpError) setError(signUpError.message);
        ...
        // We only insert manually if they use Email/Password. 
        // If they use Google, our new SQL Trigger does it automatically!
        await supabase.from("profiles").insert([{ id: (await supabase.auth.getUser()).data.user?.id, username }]);
  ...
  const handleOAuth = async (provider: 'google' | 'facebook') => {
    const { error } = await supabase.auth.signInWithOAuth({
      provider,
      options: {
        redirectTo: window.location.origin, // Sends them back to the page they were on!
      }
    });
      ...
      <div className="relative bg-surface border border-surface-border shadow-2xl rounded-2xl p-8 max-w-sm w-full animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex flex-col gap-3 mb-6">
          <button 
            ...
            className="w-full bg-white hover:bg-gray-100 text-black font-bold py-3 px-4 rounded-xl transition-colors flex items-center justify-center gap-3 shadow-sm cursor-pointer"
          >
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-foreground/40 text-xs font-bold uppercase">Or continue with Email</span>
          <div className="h-px bg-surface-border flex-1"></div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button type="submit" disabled={loading} className="w-full bg-neon-cyan hover:bg-neon-cyan/80 text-black font-bold py-3 rounded-xl mt-2 transition-colors cursor-pointer disabled:cursor-not-allowed">
